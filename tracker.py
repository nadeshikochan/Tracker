...
class SmartTracker:
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                time.sleep(1)
                now = time.time()
                loop_gap = now - self.last_loop_timestamp
                # 如果间隔超过 5 秒 (理论上应该是1秒多)，说明系统休眠过
                if loop_gap > 5:
                    print(f"💤 检测到系统休眠/挂起 (中断时长: {int(loop_gap)}s)")
                    # 1. 强制结算休眠前的任务
                    # 结束时间 = 休眠开始前的时间 (即 last_loop_timestamp)
                    self._commit_log(
                        self.stable_process,
                        self.stable_title,
                        self.stable_url,
                        self.stable_start_time,
                        self.last_loop_timestamp
                    )
                    # 2. 重置状态，将"现在"视为新任务的起点
                    # 这样休眠的那段时间就不会被计入任何任务
                    t, p, u = self.collector.get_active_window_info()
                    if t:
                        self.stable_title = t
    ...
    def _commit_log(self, process, title, url, start_ts, end_ts):
        ...
        if dt_start.date() != dt_end.date():
            # 计算午夜分界点 (次日 00:00:00)
            # 例如: start=23:50, end=次日00:10。 midnight 就是次日00:00
            midnight_date = dt_end.date()  # 取结束那天
            midnight_dt = datetime.combine(midnight_date, datetime.min.time())
            midnight_ts = midnight_dt.timestamp()

            print(f" 触发跨天切割: {process} at {midnight_dt}")

            # 递归调用：保存上半段 (start -> midnight)
            self._commit_log(process, title, url, start_ts, midnight_ts)

            # 递归调用：保存下半段 (midnight -> end)
            self._commit_log(process, title, url, midnight_ts, end_ts)
            return
